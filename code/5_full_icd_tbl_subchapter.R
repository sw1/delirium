...
# create icd groups based on reference
# will use these groups along with icd hierarchical groups
icds_enceph <- unique(c(icd10_lookup  %>% 
                          filter(str_detect(long_desc,'encephalo')) 
                        %>% select(code) %>% unlist(),
                        icd9_lookup  %>% 
                          filter(str_detect(long_desc,'encephalo')) 
                        %>% select(code) %>% unlist()))
icds_del <- unique(c(icd10_lookup  %>% 
                       filter(str_detect(long_desc,'delir'), 
                              !str_detect(long_desc,'tox|alc|coc|withdr|opioid|subst')) 
                     %>% select(code) %>% unlist(),
                     icd9_lookup  %>% 
                       filter(str_detect(long_desc,'delir'), 
                              !str_detect(long_desc,'tox|alc|coc|withdr|opioid|subst')) 
                     %>% select(code) %>% unlist()))
icds_del_tox <- unique(c(icd10_lookup  %>% 
                           filter(str_detect(long_desc,'delir'), 
                                  str_detect(long_desc,'tox|alc|coc|withdr|opioid|subst')) 
                         %>% select(code) %>% unlist(),
                         icd9_lookup  %>% 
...
for (ss in subsets){
  ...
  icd_tbl <-  icd9_lookup %>% mutate(code_type='icd9') %>% 
    ...
    select(code,code_type,sub_chapter=contains(ss)) %>%
    mutate(sub_chapter = case_when(
      code %in% icds_del_tox ~ 'delirium due to toxin',
      code %in% icds_enceph ~ 'encephalopathy',
      code %in% icds_del ~ 'delirium',
      .default = sub_chapter)) %>%
    distinct()
  ...
  tbl_full <- icd_full %>% 
    ...
    summarize(date_adm=min(date_adm,na.rm=TRUE),
              ...
              icd_sub_chapter=list(na.omit(sub_chapter)))
