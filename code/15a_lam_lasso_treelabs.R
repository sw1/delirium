...
train <- read_csv(file.path(path,'to_python','tbl_allnotes.csv.gz')) %>%
...
dv_train <- read_rds(file.path(path,'data_out','14_lam_doc_vectors_train.rds')) 
dv_test <- read_rds(file.path(path,'data_out',
                              '14_lam_doc_vectors_heldout_expert.rds')) 
...
for (i in seq_along(labs)){
  ...
  print(tbl_res %>% select(-fn),n=Inf)
  
}

write_csv(tbl_res,file.path(path,'res','15_lam_lasso_results.csv.gz'))
