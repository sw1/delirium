...
res <- tibble(labs) %>%
  ...
  mutate(b_acc=0,prec=0,rec=0,f1=0,prop1=0)
...
for (i in seq_along(labs)){
  ...
  lambda <- logistic_reg(
    ...
    tune_grid(
      recipe(y ~ .,data=x_train) ,
      resamples=vfold_cv(x_train,strata=y,v=all_cores),
      grid=5,
  ...
  res <- res %>%
    rows_update(as_tibble_row(perf) %>% mutate(fn=lab),by='fn')
...
write_rds(fits,file.path(path,'res','15_lasso_fits.rds'))
