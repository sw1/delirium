...
for(ss in subsets){
  ...
  haobo <- haobo_train %>% bind_rows(haobo_test)
  
  write_csv(ids_test,file.path(path,'data_in',
                               sprintf('test_set_sprf_%s.csv.gz',ss)),
            col_names=TRUE)
  
  ds <- 'count_del'
  
  cat(sprintf('Running workflow for %s.\n',ds))
  ...
  haobo_pre <- haobo_pre %>%
    ...
    select(where(~sum(if_else(.x > 0,1,0)) >= 10)) %>%
    select(id, label,
           starts_with('icd_'),
           starts_with('los'),
           starts_with('count_')) %>%
  ...
  write_rds(list(data=haobo_pre,fit=fit,wf=wf,
            ...
            file.path(path,'data_in',paste0('fit_rf_',ds,sprintf('_%s.rds',ss))))
