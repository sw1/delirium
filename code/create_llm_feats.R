...
create_fig <- function(tbl,param,label_type,n_feats=25,trim=40){
  ...
  icd_tbl <- icd9_lookup %>% 
    ...
    mutate(feature = paste0('icd_',feature),
           short_desc = substr(short_desc,1,trim)) %>%
    group_by(feature) %>%
...
create_fig_tptn <- function(tbl,param,n_feats=25,trim=40){
  ...
  tbl <- tbl %>% filter(str_detect(preds_type,'True'))
  
  feature_subset <- tbl 
  
  feature_subset <- feature_subset %>%
    distinct(feature,preds_type,!!param,.keep_all = TRUE) %>%
    group_by(feature,!!param) %>%
    reframe(diff=abs(diff(mean))) %>%
    arrange(desc(diff)) %>%
    distinct(feature) %>%
    slice_head(n=n_feats) %>%
    pull(feature)
  ...
  feature_order <- tbl %>%
    ...
    group_by(feature,!!param) %>%
    mutate(mean=mean/sum(mean)) %>%
    filter(preds_type == 'True Positives') %>%
    arrange(desc(mean)) %>%
    distinct(feature) %>%
    pull(feature)
...
(fig <- create_fig_tptn(tbl,param='th'))
ggsave(plot = fig,
       file.path(path,'figs','feats_th_tptn.png'),width=12,height=5,dpi = 300)


(fig <- create_fig_tptn(tbl,param='pl'))
ggsave(plot = fig,
       file.path(path,'figs','feats_pl_tptn.png'),width=12,height=5,dpi = 300)


(fig <- create_fig_tptn(tbl,param='mod'))
ggsave(plot = fig,
       file.path(path,'figs','feats_mod_tptn.png'),width=12,height=5,dpi = 300)
