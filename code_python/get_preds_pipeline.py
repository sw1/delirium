...
def get_pred_trends(ds,pl,size=10,batch_size=16,shift=50):
    ...
    for o in ds:
        ...
        #subseqs['id'].append(o['id'])
        #subseqs['i'].append(j)
        #subseqs['text'].append(o['text'])
        #subseqs['labels'].append(o['labels'])
...
runs = ['fkw1_th70_fr100_pl1_lr2.0e-06_wd1.0e-01_nb16_ls0.0e+00_cw50_labpseudo',
        'fkw1_th80_fr100_pl1_lr2.0e-06_wd1.0e-01_nb16_ls0.0e+00_cw50_labpseudo',
        'fkw1_th90_fr100_pl1_lr2.0e-06_wd1.0e-01_nb16_ls0.0e+00_cw50_labpseudo']

for run in runs:
    
    th= int(run.split('_')[1][2:])
    
    datasets = {}
    dat = read_data(os.path.join(data_dir,'tbl.csv.gz'),th=th,fr=100,exp='pseudo')
    datasets['test'] = Dataset.from_dict(dat['heldout_expert'])
    ...
    for k,ds in datasets.items():
        ...
        preds = []
        scores = []
        for r in pl(KeyDataset(ds,'text')):
            preds.append(r['label'])
            scores.append(r['score'])
            print(f"{counter}/{N}",end='\r')
            counter += 1
        ...
        ds = ds.add_column('scores', scores)
        
        ds.to_pandas().to_csv(out_path, index=False, compression='gzip')
